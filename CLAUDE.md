...
- `dotfiles/mise/` — the global mise config, linked to `~/.config/mise`.
  ...
  `[bootstrap.user]` login shell, `[bootstrap.compose]`), `dotfiles.toml` (the
  `[dotfiles]` link map — which source file lands at which `$HOME` path; sources
  are `../../<pkg>/...`).
...
- `.config/mise/tasks/` — repo-local mise tasks (`code:*`, `setup:*`,
  `system:symlinks`).
...
```shell
mise bootstrap status           # everything vs declared state (any directory)
mise bootstrap --dry-run        # preview a full converge
mise bootstrap dotfiles apply   # (re)link dotfiles; `dotfiles status` shows drift
...
- **Dotfiles edits**: edit the file under `dotfiles/<pkg>/...`; it is symlinked
  ...
  `dotfiles/mise/conf.d/dotfiles.toml`, then `mise bootstrap dotfiles apply`.
