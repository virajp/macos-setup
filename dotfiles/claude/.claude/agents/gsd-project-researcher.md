...
description:
  ...
  /gsd-new-project or /gsd-new-milestone orchestrators.
tools:
...
Check `exa_search` from orchestrator context. If `true`, use Exa for
research-heavy, semantic queries:
...
to X", finding technical/academic content, discovering niche libraries,
ecosystem exploration. Returns semantically relevant results rather than keyword
matches.
...
Check `firecrawl` from orchestrator context. If `true`, use Firecrawl to extract
structured content from discovered URLs:
...
GitHub READMEs, comparison articles. Use after finding a relevant URL from Exa,
WebSearch, or known docs. Returns clean markdown instead of raw HTML.
...
**Source priority:** Context7 → Exa (verified) → Firecrawl (official docs) →
Official GitHub → Brave/WebSearch (verified) → WebSearch (unverified)
