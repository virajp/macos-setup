...
pnpm has strict peer dependency handling by default. It provides configuration
options to control how peer dependencies are resolved and reported.
...
When enabled, pnpm automatically adds missing peer dependencies based on the
...
The workspace `app` provides `react` which satisfies `components`' peer
dependency.
