...
description:
  Worktree-first dev environment manager for issues, PRs, and features with
...
> **Quick Start (worktree-first):** Start with `omc teleport` when you want an
> isolated issue/PR/feature worktree before adding any tmux/session
> orchestration:
>
> ```bash
...
Automate isolated development environments using git worktrees and tmux sessions
with Claude Code. Enables parallel work across multiple tasks, projects, and
repositories.

Canonical slash command: `/oh-my-claudecode:project-session-manager` (alias:
...
**Important:** The `repo` field is still required for cloning the git
repository. Jira tracks issues, but you work in a git repo.
...
PSM only recognizes `PROJ-123` format as Jira when `PROJ` is explicitly
configured as a `jira_project` in your aliases. This prevents false positives
from branch names like `FIX-123`.
...
The `omc teleport` command provides a lightweight alternative to full PSM
sessions. It creates git worktrees without tmux session management — ideal for
quick, isolated development.
...
Use **PSM** for full managed sessions. Use **teleport** for quick worktree
creation.
