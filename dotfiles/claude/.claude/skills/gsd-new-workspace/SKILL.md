...
argument-hint:
  "--name <name> [--repos repo1,repo2] [--path /target] [--strategy
  worktree|clone] [--branch name] [--auto]"
allowed-tools:
  - Read
  - Bash
  - Write
...
- Multi-repo orchestration: work on a subset of repos in parallel with isolated
  GSD state
...
**After this command:** `cd` into the workspace and run `/gsd-new-project` to
initialize GSD. </objective>
