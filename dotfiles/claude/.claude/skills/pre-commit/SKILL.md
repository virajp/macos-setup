...
description: >
  ...
  manually, or asks how to install or update pre-commit hooks. Also use when
  the user asks why a commit was rejected or what hooks are running.
version: 1.0.0
---
...
> NOTE that some of the hooks are repository specific, like `local-run-biome` above.
...
`user.signingkey`, `commit.gpgsign`, `tag.gpgsign`, `gpg.*` settings. These
must live in the global `~/.gitconfig`.
...
branch (for small changes) or a feature branch. 
Use `mise run dev:merge` to promote to `develop`.
