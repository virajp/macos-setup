...
pnpm's patching feature lets you modify third-party packages directly. Useful
for applying fixes before upstream releases or customizing package behavior.
...
1. **Version specificity**: Patches are tied to exact versions. Update patches
...
3. **Minimize patches**: Keep patches small and focused. Large patches are hard
   to maintain.

4. **Track upstream**: Note upstream issues/PRs so you can remove patches when
   fixed.
