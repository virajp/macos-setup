...
TypeScript `compilerOptions.paths` breaks with JIT packages. Use Node.js subpath
imports instead (TypeScript 5.4+).
...
For internal packages, prefer `tsc` over bundlers. Bundlers can mangle code
before it reaches your app's bundler, causing hard-to-debug issues.
...
Each package should have its own `tsconfig.json`. A root one causes all tasks to
miss cache when changed. Only use root `tsconfig.json` for non-package scripts.

### Avoid TypeScript Project References

They add complexity and another caching layer. Turborepo handles dependencies
better.
