...
These patterns are **never acceptable** in Effect-TS code. Each is listed with
...
**Why:** Throws bypass Effect's error channel, can't be caught with `catchTag`,
breaks type safety.
...
specific error handling downstream.
...
**Why:** Loses Effect's error handling, can't compose with other Effects, loses
tracing/metrics.
...
**Why:** `Config.secret` is deprecated. Use `Config.redacted` instead, which
provides the same functionality with better naming and can wrap any config type.
...
**Why:** Null/undefined handling is error-prone, loses the explicit "absence"
semantics.
...
**Why:** Throws exceptions, bypasses Effect's error handling, fails at runtime
...
**Why:** Requires manual layer creation, no built-in accessors, more
boilerplate.
...
**Why:** Converts recoverable errors to defects (unrecoverable), loses error
information.
...
**Why:** Race conditions, not testable, not composable, breaks referential
transparency.
