...
**Never use `catchAll` or `mapError`** when you can use `catchTag`/`catchTags`.
These preserve type information and enable precise error handling.
...
classes.** Each explicit error can have its own HTTP status.
...
Generic errors are only acceptable for **truly unrecoverable internal errors**
where:
