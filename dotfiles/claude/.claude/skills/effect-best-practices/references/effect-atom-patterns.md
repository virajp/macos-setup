...
Effect Atom is a reactive state management library that integrates with
Effect-TS. It provides atoms (reactive containers), automatic dependency
tracking, and seamless React integration.

## Core Concepts

- **Atoms**: Reactive state containers with automatic dependency tracking
- **Result**: Handles async/effectful computations with initial, success, and
  failure states
...
**Rule:** Use `Atom.keepAlive` for global state that should persist across
component unmounts.
...
When using mutation atoms with `mode: "promise"`, derive loading state from
`result.waiting` instead of managing separate `useState`:
...
Move mutation logic INTO dialog components rather than keeping it in page
components.

**Dialog owns:** mutation hook, loading state, toast notifications **Parent
provides:** data props, `onSuccess` callback
...
Mutations can specify `reactivityKeys` to automatically invalidate queries that
share the same keys — no manual `refresh()` calls needed.
...
**Use `Result.builder`** for rendering Result types. It provides a chainable API
with granular error handling and type narrowing.
