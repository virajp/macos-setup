...
name: ccg
description:
  Claude-Codex-Gemini tri-model orchestration via /ask codex + /ask gemini, then
...
then Claude synthesizes both outputs into one answer.

Use this when you want parallel external perspectives without launching tmux
team workers.
...
- If either CLI is unavailable, continue with whichever provider is available
...
- **Gemini prompt:** UX/content clarity, alternatives, edge-case usability, docs
  polish
...
> supported in Claude Code. Always use the direct CLI path via Bash tool.
...
- Fall back to Claude-only answer and state CCG external advisors were
  unavailable
