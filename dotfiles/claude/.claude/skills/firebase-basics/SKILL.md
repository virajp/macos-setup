...
description:
  Guide for setting up and using Firebase. Use this skill when the user is
  getting started with Firebase - setting up local environment, using Firebase
  for the first time, or adding Firebase to their app.
---
...
To use the Firebase CLI, you need Node.js (version 20+ required) and npm (which
comes with Node.js).
...
1. **Install a Node Version Manager:**
   - Mac/Linux: Follow the installation instructions on the
...
**Alternative: Official Installer** Download and install the LTS version from
...
The Firebase CLI documents itself. Instruct the user to use help commands to
...
- **Permission Denied (EACCES)**: If `npm install -g` fails, suggest using a
  node version manager (nvm) or `sudo` (caution advised).
