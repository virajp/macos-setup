...
Canonical JSON schemas for all messages exchanged between agents in the
...
**File:** `.omc/self-improve/tracking/raw_data.json` — top-level JSON array,
append-only.
...
**File:** `.omc/self-improve/state/iteration_state.json` — tracks
within-iteration progress.
...
- `no_improvement` — candidates existed and were tested, but all failed
  re-benchmark (no merge occurred)
- `no_winner` — all executors failed or produced non-success status (no
...
Defined in goal.md under `## Phases`. Tracked in agent-settings.json as
`current_phase`.
