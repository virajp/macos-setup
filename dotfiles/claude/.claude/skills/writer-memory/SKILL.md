...
name: writer-memory
description:
  Agentic memory system for writers - track characters, relationships, scenes,
  and themes
argument-hint:
...
Persistent memory system designed for creative writers, with first-class support
for Korean storytelling workflows.

## Overview

Writer Memory maintains context across Claude sessions for fiction writers. It
tracks:

- **Characters (캐릭터)**: Emotional arcs (감정궤도), attitudes (태도), dialogue
  tone (대사톤), speech levels
...
All data persists in `.writer-memory/memory.json` for git-friendly
collaboration.
...
The `/synopsis` command generates an emotion-focused summary using 5 essential
elements:
...
The `/validate` command checks if dialogue matches a character's established
voice.
...
1. **On Init**: Creates `.writer-memory/memory.json` with project metadata and
   empty collections
2. **Auto-Backup**: Changes are backed up before modification to
...
6. **Backup Before Major Changes**: Use `/backup` before significant story
   pivots
