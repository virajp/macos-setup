...
description:
  Configure notification integrations (Telegram, Discord, Slack) via natural
  language
triggers:
...
Set up OMC notification integrations so you're alerted when sessions end, need
input, or complete background tasks.

## Routing

Detect which provider the user wants based on their request or argument:

- If the trigger or argument contains "telegram" → follow the **Telegram**
  section
...
Set up Telegram notifications so OMC can message you when sessions end, need
input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user
through setup by asking questions with AskUserQuestion. Write the result to
...
If existing config is found, show the user what's currently configured and ask
if they want to update or reconfigure.
...
2. **Input needed** - When Claude is waiting for your response (great for
   long-running tasks)
...
Set up Discord notifications so OMC can ping you when sessions end, need input,
or complete background tasks.
...
1. **Webhook (Recommended)** - Create a webhook in your Discord channel. Simple,
   no bot needed. Just paste the URL.
2. **Bot API** - Use a Discord bot token + channel ID. More flexible, requires a
   bot application.
...
**Question:** "Paste your Discord webhook URL. To create one: Server Settings >
...
- Must start with `https://discord.com/api/webhooks/` or
...
1. **"Paste your Discord bot token"** - From discord.com/developers > Your App >
   Bot > Token
2. **"Paste the channel ID"** - Right-click channel > Copy Channel ID (requires
   Developer Mode)
...
Ask: "What is the Discord user ID to mention? (Right-click user > Copy User ID,
...
Ask: "What is the Discord role ID to mention? (Server Settings > Roles >
...
**Question:** "Custom bot display name? (Shows as the webhook sender name in
Discord)"
...
Report success or failure. If it fails, help the user debug (check URL,
permissions, etc.).
...
Set up Slack notifications so OMC can message you when sessions end, need input,
or complete background tasks.
...
**Question:** "Paste your Slack incoming webhook URL (starts with
...
Ask: "What is the Slack member ID to mention? (Click on a user's profile > More
(⋯) > Copy member ID)"
...
**Question:** "Override the default notification channel? (The webhook already
has a default channel)"

**Options:**

1. **Use webhook default (Recommended)** - Post to the channel selected during
   webhook setup
...
- `omc --openclaw` — Activates OpenClaw gateway integration (sets
  `OMC_OPENCLAW=1`)

Without these flags, configured platforms remain dormant. This prevents unwanted
notifications during development while keeping configuration persistent.
...
If the trigger or argument contains "hook", "template", or "customize messages"
→ follow this section.

### Step 1: Detect Existing Hook Config

Check if `${CLAUDE_CONFIG_DIR:-~/.claude}/omc_config.hook.json` exists. If it
does, show the current configuration. If not, explain what it does.
...
**Question:** "Enter the message template for this event (use {{variables}} for
dynamic content)"
...
2. **Yes, customize per platform** - Set different templates for Discord,
   Telegram, Slack
...
Read or create `${CLAUDE_CONFIG_DIR:-~/.claude}/omc_config.hook.json` and merge
the new settings:
...
Validate the template using `validateTemplate()` to check for unknown variables.
If any are found, warn the user and offer to correct.
...
- `/oh-my-claudecode:configure-openclaw` — Configure OpenClaw gateway
  integration
...
Configure custom webhooks and CLI commands for services beyond the native
Discord/Telegram/Slack integrations.

### Routing

If the user says "custom integration", "openclaw", "n8n", "webhook", "cli
command", or similar → follow this section.
...
**Question:** "Existing OpenClaw configuration detected. Would you like to
...
Default for OpenClaw: session-start, session-end, stop Default for n8n:
session-end, ask-user-question
...
**Step 3: Method** Ask for HTTP method (GET, POST, PUT, PATCH, DELETE). Default:
POST.

**Step 4: Headers** Ask for headers in "Name: Value" format, one per line.
Default: Content-Type: application/json

**Step 5: Body Template** Show available template variables and ask for body
template (JSON or other format).
...
**Step 6: Timeout** Ask for timeout in milliseconds (1000-60000).
Default: 10000.
...
**Question:** "What command should be executed? (single executable, no
arguments)"
...
**Question:** "Command arguments (use {{variable}} for dynamic values). Enter
one per line."
...
- `{{agentsSpawned}}`, `{{agentsCompleted}}`, `{{modesUsed}}`,
  `{{contextSummary}}`
